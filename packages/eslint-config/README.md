...
- `@ledhed2222/eslint-config` — base: `eslint:recommended` + `import/no-default-export` (off for `*.config.*` and `*.d.ts`) + `curly: ['error', 'all']` (every `if`/`else`/`for`/`while` body must use braces, even a single statement) + `no-var`/`prefer-const` (no `var`, and `let` only where actually reassigned). Needs nothing but `eslint`.
